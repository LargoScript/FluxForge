...
const RetroGrid: React.FC<BackgroundProps<GridConfig>> = ({ children, className = '', config: propsConfig, id }) => {
  ...
  return (
    <div className={`relative w-full h-full min-h-screen overflow-hidden ${className}`} style={{ backgroundColor: bgColor }}>
      ...
      <div className="absolute inset-0 overflow-hidden perspective-[500px] z-0">
        <div className="absolute inset-0 origin-bottom transform rotate-x-[60deg]">
           <div 
             className="w-[200%] h-[200%] -ml-[50%] bg-[size:40px_40px] animate-[grid-move_linear_infinite]"
             style={{
